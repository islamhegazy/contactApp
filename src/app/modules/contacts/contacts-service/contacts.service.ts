...
import { recentContact } from 'src/app/models/recent-contact';
...
ContactListChanged = new Subject<ContactList[]>();
...
private recentContacts: Api ={
  ...
  setContactList(ContactList: ContactList[]) {
   ...
   this.ContactListChanged.next(this.Contacts.data.slice())
 }
  getContactList() {
    if(this.Contacts.stautsCode == 200){
      return this.Contacts.data.slice();
    }
  }

  getRecentContacts(){
    if(this.recentContacts.stautsCode == 200){
      // console.log(this.recentContacts.data)
      return this.recentContacts.data.slice();
      
    }
  }
  addContact(newContact:ContactList) {
   this.Contacts.data.unshift(newContact);
   this.ContactListChanged.next(this.Contacts.data.slice())
 }
}
